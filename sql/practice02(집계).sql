...
-- 예) 2014년 07월 10일
select date_format(min(hire_date), '%Y년 %m월 %d일') as '최고 선임의 입사일'
  from employees;

-- 문제4.
-- 현재 이 회사의 평균 연봉은 얼마입니까?
select floor(avg(salary)) as '현재 회사 평균 연봉'
  from salaries
 where to_date >= now();
...
select (cast(date_format(now(), '%Y') as char) - cast(date_format(max(birth_date), '%Y') as char)) as '막둥이 사원',
	   (cast(date_format(now(), '%Y') as char) - cast(date_format(min(birth_date), '%Y') as char)) as '최 연장자'
  from employees;
