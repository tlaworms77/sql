...
select sum(if(gender='f', 1, 0)) as '여직원의 수',
	   sum(if(gender='m', 1, 0)) as '남직원의 수'
  from employees;
...
select count(*) as '현재 근무자 인원'
  from salaries
  where to_date > now();
  
-- 문제5.
-- 부서는 총 몇 개가 있나요?
select count(*) as '부서의 수'
  from departments;

-- 문제6.
-- 현재 부서 매니저는 몇 명이나 있나요?(역임 매너저는 제외)
select count(distinct emp_no) as '현재 매니저의 수'
  from dept_manager
  where to_date > now();
